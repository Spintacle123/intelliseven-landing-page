...
const Internship = () => {
  ...
  return (
    <div className=' justify-center mx-auto'>
       <div className="h-[230vh] flex items-center justify-center text-center mt-24">
         <div>
           ...
           >
             {getCurrentItems().map((item, index) => (
               ...
               >
                 ...
                 <div className="flex-col items-center justify-start gap-2 mt-2">
                    <div className='flex gap-1'>
                    <span className="text-yellow-300">{item.icon}</span>
                      <h3 className="font-rajdhani text-left font-thin uppercase text-[12px]">{item.name}</h3>
                    </div>
                    
                     <div className='flex ml-1'>
                    <p className='text-xs text-main uppercase text-center'>{item.batch}</p>
                 </div>
                 </div>
               </div>
             ))}
                ...
                {tabs.map((item, index) => (
                    <div key={index}>
                        <div className='text-right'>
                            <button onClick={() => setActiveTabs(index)}>
                             ...
                             ${
                                ...
                                `}>{item}</span>
                             <div className={`border-t-2 w-10 transition 
                                ${
                                    activeTabs === index ? "border-main"
                                    : "border-main/40"
                                }`}></div>
                            </button>
                        </div>
